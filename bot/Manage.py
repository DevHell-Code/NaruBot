...
class Manage(commands.Cog):
    ...
    @cog_ext.cog_slash(name='청소')
    async def _clear(self, ctx:SlashContext, *, amount:int=999999999999999999999):
        if ctx.author.guild_permissions.manage_messages:
            try:
                await ctx.channel.purge(limit=amount)
            except:
                return await ctx.send(embed=embed("삭제 실패","http://hellcode.cf 를 이용해 메시지 관리 권한과 함께 초대해주세요!",discord.Color.red()))
            if amount == 999999999999999999999:
                await ctx.send(embed=embed("삭제 완료","이 채널에 있는 모든 메시지 (매우 오래된 메시지 제외)가 삭제되었습니다!",discord.Color.green()))
            else:
                await ctx.send(embed=embed("삭제 완료",f"{amount}개의 메시지가 삭제되었습니다!",discord.Color.green()))
    ...
    async def 타임아웃_(self, ctx:SlashContext, user:discord.Member, min:int):
        if not ctx.author.guild_permissions.administrator:
    ...
    @cog_ext.cog_slash(name="타임아웃해제")
    async def 타임아웃해제(self, ctx:SlashContext, user:discord.Member):
        if not ctx.author.guild_permissions.administrator:
    ...
    @cog_ext.cog_slash(name="언밴")
    async def 언밴_(self, ctx:SlashContext, user:discord.User):
        if ctx.author.guild_permissions.ban_members:
            await ctx.guild.unban(user)
